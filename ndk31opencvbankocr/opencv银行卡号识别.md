...
1.  `findContours(binary_not,contours,RETR_EXTERNAL,CHAIN_APPROX_SIMPLE);` 必须是黑底，白字的图才能查找轮廓
2.  `bitwise_not(binary_not,binary_not);` 颜色取反
...
## 3. 银行卡数字识别（这块我换了张图片，就没必要截取银行卡图片了）

两种方式：

1. 一种是精确截取，找到银联区域通过大小比例精确的截取 （70%）
2. 粗略的截取，截取高度 1/2 - 3/4 ， 宽度 1/12  11/12    90%  （采用）
